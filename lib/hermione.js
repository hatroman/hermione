...
module.exports = class Hermione extends QEmitter {
    ...
    readTests(testPaths, browsers, options) {
        options = _.defaults(options || {}, {loadPlugins: true, passthroughEvents: true});

        options.loadPlugins && this._loadPlugins();

        const runner = Runner.create(this._config);
        options.passthroughEvents && qUtils.passthroughEvent(runner, this, _.values(RunnerEvents.getSync()));
